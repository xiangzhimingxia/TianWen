...
//    [self destructionRecordingFile];
    ...
    self.recordFileStr = nil;  //作用待定，可有可无
    self.recorder = nil; //录制多个音频，recorder滞空很关键，让recorder每次重新懒加载
...
//        NSString *filePath = [path stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.wav",[NSString getCurrentTimesAboutTimestamp]]];
//        self.recordFileUrl = [NSURL fileURLWithPath:filePath];
//        NSLog(@"%@", filePath);
        
        
        NSString *filePath = [[[JLKFUtil getCacheDirectory] stringByAppendingPathComponent:[NSString getCurrentTimesAboutTimestamp]] stringByAppendingPathExtension:@"wav"];
        
        self.recordFileStr = filePath;
        self.recordFileUrl = [NSURL fileURLWithPath:filePath];
        NSLog(@"%@", filePath);
...
- (void)audioRecorderDidFinishRecording:(AVAudioRecorder *)recorder successfully:(BOOL)flag {
    ...
    //行不通，当stop时，currentTime已清空
//    double currentTime = self.recorder.currentTime;
//    if (currentTime < 1) {
//
//        if ([self.delegate respondsToSelector:@selector(getBackFinishFileURL:AndRecordTimeStr:)]) {
//
//            NSString *currentRecordTime = [NSString stringWithFormat:@"%.1lf''",currentTime];
//            [self.delegate getBackFinishFileURL:self.recordFileUrl AndRecordTimeStr:currentRecordTime];
//        }
//
//    }
}
